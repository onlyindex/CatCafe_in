# form vs flaskform 区别? flask 版本更新后form 的subclass 统一叫flaskform 与 wtform 中的 form 区分开了
...
class ResetEmailForm(FlaskForm):
    resetemail = StringField('New Email Address', [validators.Length(min=6, max=35)])
...
# 管理员编辑资料表单(还能编辑用户的电子邮件、用户名、确认状态和角色)
